...
fun dateStrToDigit(str: String): String {
    ...
    if ((numberOfMonth == 2) && (date[2].toInt() % 4 != 0) && (date[2].toInt() % 400 != 0))
...
fun flattenPhoneNumber(phone: String): String {
    ...
    if (result.matches(Regex("""^(\+[0-9]+)?(\([0-9]+\))?([0-9]+)$"""))) return Regex("""[()]""").replace(
        result, ""
    )
    return ""
}
